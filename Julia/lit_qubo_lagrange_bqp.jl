using JuMP, LinearAlgebra, DelimitedFiles, Ipopt, CPUTime, SparseArrays


function qubo(nome)
#readQ = readdlm("in3000_3.txt")
#readQ = readdlm("be/be120.3.3")
readQ = readdlm(nome)
...
for i=2:size(readQ,1)
	#println(readQ[i,1], " ",readQ[i,2], " " , Q[readQ[i,1],readQ[i,2]])
	Q[readQ[i,1],readQ[i,2]] = readQ[i,3]
	Q[readQ[i,2],readQ[i,1]] = readQ[i,3]
end

#println("Será que eu sai daqui??")
#Q = sparse(readQ[2:end,1], readQ[2:end,2], readQ[2:end,3])
n = size(Q,1)
epsolon_1 = 10^(-3)
lambda = 1
rho = 5
...
f = open("resultados_x(x-1)_ipopt_bqp.txt", "a+")
...
function main()
nomes_instancias = ["beasley(bqp)/bqp1000-8.sparse.txt","beasley(bqp)/bqp1000-10.sparse.txt","beasley(bqp)/bqp100-8.sparse","beasley(bqp)/bqp250-3.sparse","beasley(bqp)/bqp500-8.sparse","beasley(bqp)/bqp1000-2.sparse.txt","beasley(bqp)/bqp100-9.sparse","beasley(bqp)/bqp250-4.sparse","beasley(bqp)/bqp500-9.sparse",
...
"beasley(bqp)/bqp1000-7.sparse.txt","beasley(bqp)/bqp2500-2.sparse.txt","beasley(bqp)/bqp250-7.sparse","beasley(bqp)/bqp50-2.sparse","beasley(bqp)/bqp2500-3.sparse.txt","beasley(bqp)/bqp250-8.sparse","beasley(bqp)/bqp50-3.sparse","beasley(bqp)/bqp1000-9.sparse.txt",
...
g = open("tempo_execucao_x(x-1)_ipopt_bqp.txt", "w+")
